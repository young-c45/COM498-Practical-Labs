...
public final class LinkedBag<T> implements BagInterface<T> {
    ...
    // Method to display chain elements from specified node
    private void displayChain(MyNode<T> node, String prefix) {
        // Runs if node exists
        if (node != null) {
            // Displays current node
            System.out.println(prefix + node.getData());
            // Displays chain from next node
            displayChain(node.getNext(), prefix);
        }
    }

    // Method to display all elements in bag
    public void display(String prefix) {
        // Displays chain from first node
        displayChain(firstNode, prefix);
    }
